...
def moss(ticket):
    ...
    def download_data(startDate,endDate, metrics, dimensions, eventAction= '', completeData = False):
    ...
    #     eventAction = 'ga:eventAction=='+eventAction
        ...
        if completeData:
            ...
            data = service.data().ga().get( 
                                ...
                                end_date=endDate,
                                metrics=metrics,
                                dimensions = dimension, filters = eventAction ).execute()
    ...
    def download(data,device,  deviceSplit = True):
        ...
        def dataClean(df):
            df['experimentVariant'] = df['eventLabel'].apply(like_function)
            df['experimentName'] = df['eventAction'] 
            return df
        
        if deviceSplit:
            df_overall           = download_data(startDate, endDate, met, dim1,title)
            output['Overall'] = dataClean(df_overall)
            
            #             Devices
            df_device            = download_data(startDate, endDate, met, dim2,title)
            output['Device'] = dataClean(df_device)
            
            #             UserType
            df_user_type         = download_data(startDate, endDate, met, dim3,title)
            ...
            for i in range(len(days)):
                ...
                print(day)
                try:
                    day_device_int = download_data(startDate, day, met,dim6,title)
            ...
            output['Device'] = dataClean(df_overall)
            ...
            for i in range(len(days)):
                ...
                print(day)
                try:
                    day_device_int = download_data(startDate, day, met, dim5,title)
                    day_device_int['date'] = day
        ...
        if noData == True :
            ...
            if singleData:
                if multiDevice:
                    ...
                    print(data)
                    output = download(data,ticket['Devices'][0].lower(),deviceSplit = False)
            else:
                output = {}
    ...
    if ticket['ForecastModule']:  
        ...
        df_forecast = download_data('2010-01-01', 'today', met, ['ga:yearMonth'], completeData=True)
    ...
    df_new['Revenue Uplift']= df_new['Revenue Uplift'].apply(lambda x:'\xA3'+'{:20,.2f}'.format(x))
...
########################################################################################################################################
    ...
    file_name = f"./Template/{ticket['Title'].replace('/','').replace('&','')}.html"
    ...
    pdfkit.from_file(file_name, f"./PDF/{ticket['Title'].replace('/','').replace('&','')}.pdf", options=options)
