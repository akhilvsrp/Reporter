...
def chums(ticket):
    ...
    #     eventAction = 'ga:eventAction=='+eventAction
        ...
        if completeData:
            ...
            data = service.data().ga().get( 
                                ...
                                end_date=endDate,
                                metrics=metrics,
                                dimensions = dimension, filters = 'ga:eventCategory==SiteGainer Events').execute()
    ...
    def download(data,device, deviceSplit = True):
        dim1 = ['ga:eventAction','ga:eventLabel', 'ga:eventCategory']
        dim2 = ['ga:eventAction','ga:eventLabel', 'ga:eventCategory','ga:deviceCategory']
        dim3 = ['ga:eventAction','ga:eventLabel', 'ga:eventCategory','ga:userType'      ]
        dim4 = ['ga:eventAction','ga:eventLabel', 'ga:eventCategory','ga:deviceCategory','ga:userType']
        #Line Graphs
        dim5 = ['ga:eventAction','ga:eventLabel', 'ga:eventCategory']
        dim6 = ['ga:eventAction','ga:eventLabel', 'ga:eventCategory', 'ga:deviceCategory']
        dim7 = ['ga:yearMonth']
        ...
        def dataClean(df):
            df = df[df['eventLabel'].str.contains(testId)]
            df['experimentVariant'] = df['eventAction'].apply(like_function)
            df['experimentName'] = df['eventLabel']
            return df
        
        df_all_site           = download_data(startDate, endDate, met, [],completeData = True)
        total_users = int(df_all_site['users'][0])
        output["Total_Users"] = total_users  
        
        if deviceSplit:
            df_overall           = download_data(startDate, endDate, met, dim1, testId)
            df_overall = dataClean(df_overall)
            output['Overall'] = df_overall
            
            #             Devices
            df_device            = download_data(startDate, endDate, met, dim2,testId)
            df_device = dataClean(df_device)
            output['Device'] = df_device
            
            #             UserType
            df_user_type         = download_data(startDate, endDate, met, dim3,testId)
            df_user_type = dataClean(df_user_type)
            output['User'] = df_user_type
            ...
            for i in range(len(days)):
                day = days[i]
                print(day)
                day_device_int = download_data(startDate, day, met, dim6,testId)
            ...
            df_overall = dataClean(df_overall)
            output['Device'] = df_overall

            #             UserType
            df_user_type         = download_data(startDate, endDate, met, dim3,testId)
            df_user_type['deviceCategory'] =  device
            df_user_type = dataClean(df_user_type)
            output['DeviceUser'] = df_user_type
        ...
        if noData == True :
            ...
            if singleData:
                if multiDevice:
                    ...
                    print(data)
                    output = download(data,ticket['Devices'][0],deviceSplit = False)
            else:
                output = {}
    ...
    if 'Total_Users' in daat.keys():
        total_users = daat['Total_Users']
    else:
        total_users = daat[list(daat.keys())[0]]['Total_Users']

    revenue = []
    for i in data.keys():
        val = uplift_calculation(data[i],total_users,startDate, endDate )
    ...
    # if df_new[''].str.startswith('var2').sum() > 0 :
    #     df_new = df_new[(df_new[''].str.startswith('var1')) | (df_new[''].str.startswith('var2'))]
    # else:
    #     df_new = df_new[(df_new[''] == 'Overall' )|( df_new[''].str.startswith('Device'))]
    ...
    if ticket['ForecastModule']:   
        forecastGraph = False
        try:
            df_max = df_new[(df_new['inter'] == 2)].iloc[0]
            if float(df_max['Chance of Being Best'].replace('%','')) > 90:
                result,annual_revenue_forecast,annual_revenue_uplift,revenue_diff=revenue_forecast(df_forecast[['transactionRevenue']],df_max['Uplift']/100)
