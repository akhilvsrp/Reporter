...
def blindsDirect(ticket):
    ...
    def download(data,device,deviceSplit = True):
        ...
        if dimension is None:
            ...
            dim_val = 'ga:dimension'+dimension
        
        dim1 = [dim_val]
        dim2 = [dim_val,'ga:deviceCategory']
        dim3 = [dim_val,'ga:userType'      ]
        dim4 = [dim_val,'ga:deviceCategory','ga:userType']
        #Line Graphs
        dim5 = [dim_val,'ga:date']
        dim6 = [dim_val,'ga:deviceCategory']
        dim7 = ['ga:yearMonth']
        ...
        def dataClean(df):
            def is_value(x):
                return x == x 
            df['experimentVariant'] = df[dim_val.split(':')[1]].map(id_map)
            df = df[df['experimentVariant'].apply(is_value)]
            df['experimentName'] = title
            return df
    ...
    ticket['desktoptestTitle'] = ticket['Title']
    ...
    file_name = f"./Template/{ticket['Title']}.html"
    ...
    pdfkit.from_file(file_name, f"./PDF/{ticket['Title']}.pdf", options=options)
